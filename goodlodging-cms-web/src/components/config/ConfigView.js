...
const ConfigView = ({ onConfigChange, initialConfig = {} }) => {
    ...
    const handleChange = (type, value) => {
        ...
        if (newConfig.roomType) {
            updatedPayload.roomType = newConfig.roomType.id === 1
                ? getValuesExcludingId(1, postTypeConstants) // [1, 2] khi id=1
                : [newConfig.roomType.value]; // [1] hoặc [2] khi id!=1
        }
        if (newConfig.roomPrice) {
            updatedPayload.minRent = newConfig.roomPrice.min;
            updatedPayload.maxRent = newConfig.roomPrice.max;
        }
    ...
    useEffect(() => {
        const updatedPayload = {};
        if (selectedConfig.roomType) {
            updatedPayload.roomType = selectedConfig.roomType.id === 1
                ? getValuesExcludingId(1, postTypeConstants) // [1, 2]
                : [selectedConfig.roomType.value]; // [1] hoặc [2]
        }
        if (selectedConfig.roomPrice) {
            updatedPayload.minRent = selectedConfig.roomPrice.min;
            updatedPayload.maxRent = selectedConfig.roomPrice.max;
        }
    ...
    const handleReset = () => {
        ...
        const updatedPayload = {};
        if (resetRoomType) {
            updatedPayload.roomType = resetRoomType.id === 1
                ? getValuesExcludingId(1, postTypeConstants) // [1, 2]
                : [resetRoomType.value]; // [1] hoặc [2]
        }
        if (resetRoomPrice) {
            updatedPayload.minRent = resetRoomPrice.min;
            updatedPayload.maxRent = resetRoomPrice.max;
        }
